...
FROM player_test
WHERE metrics->'$.runStop' != '0' AND position in ('CB')
EOT;
...
FROM player_test
WHERE metrics->'$.insideRush' != '0' AND position in ('CB')
EOT;
        $stmt= $this->db->query($sql);
        $result = $stmt->execute();
        if (! $result instanceof ResultInterface || ! $result->isQueryResult()) {
...
FROM player_test
WHERE metrics->'$.coverage' != '0' AND position in ('CB')
EOT;
        $stmt= $this->db->query($sql);
        $result = $stmt->execute();
        if (! $result instanceof ResultInterface || ! $result->isQueryResult()) {
        ...
        $sql = <<<EOT
SELECT id, first_name, last_name, lpad(json_unquote(metrics->'$.pressCoverage'),6,'0'),  PERCENT_RANK() OVER (ORDER BY lpad(json_unquote(metrics->'$.pressCoverage'),6,'0')) percentile_rank
FROM player_test
WHERE metrics->'$.pressCoverage' != '0' AND position in ('CB')
EOT;
        $stmt= $this->db->query($sql);
        $result = $stmt->execute();
        if (! $result instanceof ResultInterface || ! $result->isQueryResult()) {
        ...
        $pressCoverage = [];
        foreach($resultSet as $row) {
            $pressCoverage[$row->id] = $row->percentile_rank;
        }
        print "press Coverage built\n";

        /*****************************************************************************/
        $sql = <<<EOT
SELECT id, first_name, last_name, lpad(json_unquote(metrics->'$.outsideCoverage'),6,'0'),  PERCENT_RANK() OVER (ORDER BY lpad(json_unquote(metrics->'$.outsideCoverage'),6,'0')) percentile_rank
FROM player_test
WHERE metrics->'$.outsideCoverage' != '0' AND position in ('CB')
EOT;
        $stmt= $this->db->query($sql);
        $result = $stmt->execute();
        if (! $result instanceof ResultInterface || ! $result->isQueryResult()) {
        ...
        $outsideCoverage = [];
        foreach($resultSet as $row) {
            $outsideCoverage[$row->id] = $row->percentile_rank;
        }
        print "outside coverage built\n";

        /*****************************************************************************/
        $sql = <<<EOT
SELECT id, first_name, last_name, lpad(json_unquote(metrics->'$.slotCoverage'),6,'0'),  PERCENT_RANK() OVER (ORDER BY lpad(json_unquote(metrics->'$.slotCoverage'),6,'0')) percentile_rank
FROM player_test
WHERE metrics->'$.slotCoverage' != '0' AND position in ('CB')
EOT;
        $stmt= $this->db->query($sql);
        $result = $stmt->execute();
        if (! $result instanceof ResultInterface || ! $result->isQueryResult()) {
        ...
        $slotCoverage = [];
        foreach($resultSet as $row) {
            $slotCoverage[$row->id] = $row->percentile_rank;
        }
        print "slot coverage built\n";
        ...
        foreach ($players as $player) {
            ...
            $data['outsideCoverage'] = (array_key_exists($id, $outsideCoverage)) ? $outsideCoverage[$id] * 100 : "";
            $data['slotCoverage'] = (array_key_exists($id, $slotCoverage)) ? $slotCoverage[$id] * 100 : "";
            $data['pressCoverage'] = (array_key_exists($id, $pressCoverage)) ? $pressCoverage[$id] * 100 : "";
